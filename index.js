...
const routes = [
  ...
  },
  {
    method: 'GET',
    path: '/routes',
    description: 'Retrieve a list of all registered routes and their descriptions'
  }
];
...
app.post('/registrations', (req, res) => {
  ...
  db.run('INSERT INTO Registration (name, email, date_of_birth) VALUES (?, ?, ?)', [name, email, date_of_birth], function (err) {
    if (err) {
...
app.put('/registrations/:id', (req, res) => {
  const id = req.params.id;
  const { name, email, date_of_birth } = req.body;
  db.run('UPDATE Registration SET name = ?, email = ?, date_of_birth = ? WHERE id = ?', [name, email, date_of_birth, id], function (err) {
    if (err) {
      if (err.code === 'SQLITE_CONSTRAINT') {
        return res.status(400).json({ error: err.message });
      }
